...
// Data Handling - Reusing existing images for now
const TOPICS = [
...
const DIFFICULTIES = [
    { id: "Easy", label: "Easy", desc: "Common" },
    { id: "Medium", label: "Medium", desc: "Technical" },
    { id: "Hard", label: "Hard", desc: "Obscure" }
] as const;
...
const SetupScreen: React.FC<SetupScreenProps> = ({ onComplete, onBack }) => {
    ...
    const getButtonText = () => {
        if (viewMode === 'MACRO') return "Configure Context";

        const count = subTopicSelection.length;
        if (count === 0) return `Start ${selectedTopic}`;
        if (count === 1) return `Start ${subTopicSelection[0]}`;
            ...
            <div className="mb-4 shrink-0 px-1 flex items-center h-14 relative z-10 animate-in fade-in slide-in-from-top-2 duration-500">
                ...
                <div>
                    <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight leading-none drop-shadow-md">
                        {viewMode === 'MACRO' ? 'Context Selection' : selectedTopic}
                    </h1>
                    <p className="text-slate-grey text-xs sm:text-sm font-medium mt-1">
                        {viewMode === 'MACRO' ? 'Select your domain of knowledge.' : 'Refine specific subjects.'}
                    </p>
                </div>
            </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 h-[300px] animate-in slide-in-from-left-8 fade-in duration-500">
                        {TOPICS.map((topic) => (
                            <button
                                ...
                                className={cn(
                                    "relative rounded-2xl flex flex-col justify-end text-left transition-all duration-300 border border-white/5 hover:border-analogyai-secondary/50 group overflow-hidden outline-none focus:ring-2 focus:ring-analogyai-secondary/50",
                                    topic.span
                                )}
                            >
                            ...
                            <button
                                ...
                                className={cn(
                                    ...
                                    subTopicSelection.length === 0
                                        ? "border-analogyai-secondary shadow-[0_0_20px_-5px_rgba(34,211,238,0.5)] scale-[1.01]"
                                        : "border-white/10 opacity-60 hover:opacity-100 hover:border-white/30"
                                )}
                            >
                                {/* Background Image (Unblurred slice) */}
                                <div className="absolute inset-0">
                                    <div className="absolute inset-0 bg-gradient-to-r from-cyan-950/80 to-black/80 z-10" />
                                    <img
                                ...
                                <div className="relative z-20 flex items-center justify-between px-6 h-full">
                                    <div className="flex items-center gap-3">
                                        <Aperture className={cn("h-5 w-5", subTopicSelection.length === 0 ? "text-cyan-300" : "text-zinc-500")} />
                                ...
                                return (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            active
                                                ? "border-cyan-400 shadow-lg scale-[1.02] z-10"
                                    ...
                                    >
                                        <div className="absolute inset-0">
                                            ...
                                            <div className={cn(
                                                ...
                                                active ? "bg-cyan-900/30" : "bg-black/60"
                                            )} />
                                        </div>
                                ...
                                return (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            isSelected
                                                ? "bg-analogyai-secondary text-white shadow-md"
